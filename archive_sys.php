...
for ($i = 0;$i < $nombre_ligne;$i++) { //affichage des objectifs
    ...
    if ($_SESSION["role_user"] == 1 || $double_tab[$i][4] == 1 or $_SESSION["role_user"] == 3) {
        ...
        echo "<td class='sms'>
        <center>
            <button style=\"background-color:gray;\" type=\"button\" onclick=\"', this)\"> <span class=\" icon-mail\"> Messagerie &#128172; </span></button>
        </center>
    </td>
    <td>
        <center>
            <a><button style=\"background-color:gray;\" class=\"objectif-acceder\" > Acceder </button></a>
        </center>
    </td>
    <td>
        ...
        <center>
            <button class=\"button_ajouter-objectif\" type=\"button\" onclick=\"openDialog(\'dialog6\', this)\">Dé-archiver cet objectif</button>
            <div role=\"dialog\" id=\"dialog6\" aria-labelledby=\"dialog1_label\" aria-modal=\"true\" class=\"hidden\">
                <p class='popup-txt'> Voulez vous rendre de nouveau utilisable cet objectif ? Il sera visible par tous les membres suivant cet enfant et apparaitra dans les statistiques  </p>
                <div style=\"display:flex; justify-content: space-evenly;\">
                    <button class=\"popup-btn\" type=\"button\" onclick=\"closeDialog(this)\">Annuler</button>
                    <a href=\"archive_sys.php?id_putback=' . $double_tab[$i][5] . '\"><button class=\"popup-btn\"> Valider </button></a>
                </div>
            </div>
        </center>
    </td>
    <td>
        ...
        <center>
            <button class=\"button_ajouter-objectif\" type=\"button\" onclick=\"openDialog(\'dialog7\', this)\">Supprimer cet objectif</button>
            <div role=\"dialog\" id=\"dialog7\" aria-labelledby=\"dialog1_label\" aria-modal=\"true\" class=\"hidden\">
                <p class='popup-txt'> Voulez vous supprimer définitivement cet objectif ? cela le retirera des statistiques, supprimera tous ses messages, supprimera aussi toutes ses récompenses.  </p>
                <div style=\"display:flex; justify-content: space-evenly;\">
                    <button class=\"popup-btn\" type=\"button\" onclick=\"closeDialog(this)\">Annuler</button>
                    <a href=\"appel_fonction.php?appel=supprime_objectif&id_sys=' . $double_tab[$i][5] . '\"><button class=\"popup-btn\"> Valider </button></a>
                </div>
            </div>
        </center>
    </td>
...
while ($tuteur = $res->fetch()) {
    ...
    <td class='Profil'>
        <form action='groupe_validation.php?id_enfant=$getid&id_membre=$tuteur[id_membre]' method='post'>
