...
try {
    ...
    'intitule' => $nom, 'nb_jeton' => $total_jeton, 'duree' => $duree, 'jeton' => $jeton, 'prio' => $prio, 'etat' => 0, 'id_membre' => $_SESSION['logged_user'], 'id_enfant' => $_SESSION['id_enfant'], 'type_sys' => $_SESSION['type_sys']));
    // $req->debugDumpParams();
    // exit();
    if ($req == false) {
        $req->debugDumpParams();
